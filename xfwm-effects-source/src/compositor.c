...
paint_win (CWindow *cw, XserverRegion region, Picture paint_buffer, gboolean solid_part)
{
    ...
    // effect:
    // we assume a dark theme is being used, unless tint is being
...
get_xfconf_settings(ScreenInfo *screen_info)
{
    ...
    // the on-off switch for xfwm effect
    ...
    //effect color to blend in
    if(xfconf_channel_has_property(window_effects, "/effect-color"))
    {
		
		gdouble rc, gc, bc, ac;
		xfconf_channel_get_array (window_effects,
								  "/effect-color",
								  G_TYPE_DOUBLE, &rc,
								  G_TYPE_DOUBLE, &gc,
								  G_TYPE_DOUBLE, &bc,
								  G_TYPE_DOUBLE, &ac,
								  G_TYPE_INVALID);
		GdkRGBA blend_color = { (gdouble) rc, (gdouble) gc, (gdouble) bc, (gdouble) ac };
        if (!gdk_rgba_equal (&screen_info->blend_color, &blend_color))
        {
            screen_info->blend_color = blend_color;
            setting_changed = True;
        }
...
need_effect_update(ScreenInfo *screen_info)
{
    ...
    //the code for get_focused_window (now removed) can be seen in
    // previous versions; it was causing crashes because (I think)
    // window list was mutating while XQueryTree was being used; 
...
paint_effect(ScreenInfo *screen_info)
{
    ...
    {
		//NOTE that color_value here determines the amount of color to mix in
        gdouble color_value = (gdouble) screen_info->white_amount / 100;
        tmpPicture = None;
        
        //color to blend in for effect (from prefs)
        gdouble red_value = screen_info->blend_color.red;        
        gdouble green_value = screen_info->blend_color.green;
        gdouble blue_value = screen_info->blend_color.blue;
        //sanity
        if (red_value < 0 || red_value > 1) red_value = 1.0;
        if (green_value < 0 || green_value > 1) green_value = 1.0;
        if (blue_value < 0 || blue_value > 1) blue_value = 1.0;
        
        //create 'effect color' picture
        tmpPicture = solid_picture (screen_info, TRUE,
                            0.1,  // alpha //0.5
                            red_value * color_value, //color_value,  // red
                            green_value * color_value, //color_value,  // green
                            blue_value * color_value); //color_value); // blue ; rgb == orange
		//alpha mask
