...
// effect:
#include <Imlib2.h> //img library
#include <xfconf/xfconf.h> //channel settings
#include <stdio.h>
#include <memory.h> //memcpy
#include <time.h> //log_time
...
#define MONITOR_ROOT_PIXMAP   1
#endif /* MONITOR_ROOT_PIXMAP */
// effect: box blur kernel
static XFixed convolution_blur1[] = {
	//must convert to XFixed with XDoubleToFixed()
	//matrix size
	XDoubleToFixed(5), XDoubleToFixed(5),
	//matrix
	XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ),
	XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ),
	XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ),
	XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ),
	XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ), XDoubleToFixed( .04 ),
...
// effect: global bool
gboolean need_load_xfconf_settings = True;
...
struct _CWindow
{
    ...
    // effects
    // _GTK_FRAME_EXTENTS(CARDINAL)
    gboolean has_gtk_frame_extents;
    gint gtk_frame_l;
    gint gtk_frame_r;
    gint gtk_frame_t;
    gint gtk_frame_b;
...
log_time(char * c)
{
    ...
    printf("\n %s --> %ld.%ld \n", c, tv.tv_sec % SEC_WRAP, tv.tv_usec / 1000);
}
...
paint_root (ScreenInfo *screen_info, Picture paint_buffer)
{
    ...
    {
        // root_tile method pulls out the background image data 
        // stored in 'root' window as a property
        screen_info->rootTile = root_tile (screen_info);
        g_return_if_fail (screen_info->rootTile != None);
    }
    //paints picture of root window pixmap onto the picture buffer
...
// effects:
// gtk_frame_extents / check_gtk_frame_extents helper methods determine
// size of window frame for GTK windows (minus empty 'grabber' space)

static gboolean
gtk_frame_extents(Display *dpy, Window child, CWindow *cw)
{    
    guchar *data = None;
    Atom type_return = None;
    gulong nitems_return = None;
    gulong bytes_after_return = None;
    gint format_return = 0;

    cw->has_gtk_frame_extents = False;
    Atom atom = None;
    atom = XInternAtom (dpy, "_GTK_FRAME_EXTENTS", FALSE);

    if (Success == XGetWindowProperty (dpy, child, atom,
                          0L, G_MAXLONG, False, XA_CARDINAL, &type_return,
                          ...
                          &data))
    {
                ...
                (nitems_return == 4) && (data))
        {
            ...
            // _GTK_FRAME_EXTENTS goes: left, right, top, bottom
            cw->gtk_frame_l = ldata[0];
            cw->gtk_frame_r = ldata[1];
            cw->gtk_frame_t = ldata[2];
            cw->gtk_frame_b = ldata[3];
            cw->has_gtk_frame_extents = True;
            
            //fprintf(stderr, "win:%lx l:%d r:%d, t:%d, b:%d \n", cw->id, cw->gtk_frame_l, cw->gtk_frame_r, cw->gtk_frame_t, cw->gtk_frame_b);
        }
    }
    else
    {
        //fprintf(stderr, "win:%lx has NO gtk_extents", cw->id);
    }
    if (data)
        XFree (data);
        
    return cw->has_gtk_frame_extents ? True : False;
}

static gboolean
check_gtk_frame_extents(Display *dpy, CWindow *cw)
{
    ...
    unsigned int n = 0; //number of children
    gboolean has_gtk_frame = False;
    
    //check if this window (cw) has gtk frame extents
    if (gtk_frame_extents(dpy, cw->id, cw))
        return True;
            
    // examine window's tree; one subwindow might have gtk frame extents
    // (e.g. GTK3 with client side decorations)
    if (XQueryTree(dpy, cw->id, &root, &parent, &children, &n))
    {
        if (children)
        {
            for (int i = 0; i < n; i++)
            {
                //fprintf(stderr, "cw:%lx chld:%lx \n", cw->id, children[i]);
                if (gtk_frame_extents(dpy, children[i], cw))
                {
                    has_gtk_frame = True;
                }
            }
    ...
    //cw->has_gtk_frame_extents ? printf("win: %lx has frame extents \n", cw->id) : printf("win: %lx has NO frame extents \n", cw->id);
    return has_gtk_frame;
}
...
paint_win (CWindow *cw, XserverRegion region, Picture paint_buffer, gboolean solid_part)
{
    ...
    //log_time("paint window");
    //printf("\n win: %lu xywh: %d, %d, %d, %d \n", cw->id, cw->attr.x, cw->attr.y, cw->attr.width, cw->attr.height);
    screen_info = cw->screen_info;
    display_info = screen_info->display_info;
    paint_solid = ((solid_part) && WIN_IS_OPAQUE(cw));
    
    // effect:
    // we assume a dark theme is being used, unless white is being
    // composited into the effect; then we assume a light theme;
    // this also determines which compositing operation is used to 
    // re-paint window
    gboolean dark_theme = True;
	if (screen_info->white_amount && screen_info->white_amount > 0)
    {
		dark_theme = False;
	}
	
   
    // if has FRAME and frame is NOT OPAQUE //
    ...
    if (WIN_HAS_FRAME(cw) && (screen_info->params->frame_opacity < 100))
    {
        ...
        // paint NOT OPAQUE FRAME

        if (!solid_part)
        {
            // no cache, so make cache of ALPHA BLACK
            ...
            if (!cw->alphaBorderPict)
            {
                ...
                //use for effect instead of frame
                if (screen_info->effect_picture)
                {
					frame_opacity = 1 - frame_opacity;
				}
            ...
            // PAINT parts of NOT OPAQUE FRAME
            
            if (screen_info->effect_picture)
            {
                // Top Border (title bar) //

                XRenderComposite (display_info->dpy, PictOpSrc, cw->picture, 0, paint_buffer,
                            0, 0, 0, 0,
                            frame_x, frame_y, frame_width, frame_y + frame_height);

                XRenderComposite (display_info->dpy, PictOpOver, 
							screen_info->effect_picture, cw->alphaBorderPict, paint_buffer,
                            frame_x, frame_y, frame_width, frame_y + frame_height,
                            frame_x, frame_y, frame_width, frame_y + frame_height);                
                //to maintain legibility of title
                XRenderComposite (display_info->dpy, dark_theme ? PictOpHardLight : PictOpMultiply,
							cw->picture, cw->alphaBorderPict, paint_buffer,
                            0, 0, 0, 0,
                            frame_x, frame_y, frame_width, frame_y + frame_height);
				/*
                // Bottom Border //
                ...
                XRenderComposite (display_info->dpy, PictOpSrc, cw->picture, 0, paint_buffer,
                            0, frame_height - frame_bottom, 0, 0, 
                            frame_x, frame_y + frame_height - frame_bottom, frame_width, frame_bottom);
                XRenderComposite (display_info->dpy, PictOpOver, 
							screen_info->effect_picture, cw->alphaBorderPict, paint_buffer,
                            frame_x, frame_y + frame_height - frame_bottom, frame_width, frame_bottom,
                            frame_x, frame_y + frame_height - frame_bottom, frame_width, frame_bottom);
                ...
                XRenderComposite (display_info->dpy, PictOpSrc, cw->picture, 0, paint_buffer,
                            0, frame_top, 0, 0,
                            frame_x, frame_y + frame_top, frame_left, frame_height - frame_top - frame_bottom);
                XRenderComposite (display_info->dpy, PictOpOver, 
							screen_info->effect_picture, cw->alphaBorderPict, paint_buffer,
                            frame_x, frame_y + frame_top, frame_left, frame_height - frame_top - frame_bottom,
                            frame_x, frame_y + frame_top, frame_left, frame_height - frame_top - frame_bottom);
                //XRenderComposite (display_info->dpy,  dark_theme ? PictOpHardLight : PictOpMultiply, cw->picture, 0, paint_buffer,
                            //0, frame_top,
                            //0, 0,
                ...
                XRenderComposite (display_info->dpy, PictOpSrc, cw->picture, 0, paint_buffer,
                            frame_width - frame_right, frame_top, 0, 0, 
                            frame_x + frame_width - frame_right, frame_y + frame_top, 
                ...
                //XRenderComposite (display_info->dpy,  dark_theme ? PictOpHardLight : PictOpMultiply, cw->picture, 0, paint_buffer,
                            ...
                            //0, 0, frame_x + frame_width - frame_right,
        ...
        // paint NOT OPAQUE WINDOW in frame
        ...
        else if (!solid_part)
        {
            ...
            {
                // effect: 
                // as the slider moves toward 'opaque' the window looks more 
                // and more like it does when compositing is off, and as the 
                // slider moves toward 'transparent' a blurred version of 
                // the desktop is visible "through" the window
               
                XRenderComposite (display_info->dpy, PictOpSrc, screen_info->effect_picture, 0, paint_buffer,
                                frame_x + frame_left, frame_y + frame_top,
                                frame_width - frame_left - frame_right, frame_height - frame_top - frame_bottom,
                                frame_x + frame_left, frame_y + frame_top,
                                frame_width - frame_left - frame_right, frame_height - frame_top - frame_bottom);
				
                XRenderComposite (display_info->dpy, PictOpOver, cw->picture, cw->alphaPict, paint_buffer,
                                frame_left, frame_top, 0, 0,
                                frame_x + frame_left, frame_y + frame_top,
                                frame_width - frame_left - frame_right, frame_height - frame_top - frame_bottom);

                // 'lighten' punches up light text on dark theme; 'darken' vice versa
                XRenderComposite (display_info->dpy, dark_theme ? PictOpLighten : PictOpDarken, 
								cw->picture, 0, paint_buffer,
                                frame_left, frame_top, 0, 0,
                                frame_x + frame_left, frame_y + frame_top,
                                frame_width - frame_left - frame_right, frame_height - frame_top - frame_bottom);
            }
    ...
    // NO FRAME, paint OPAQUE or NOT OPAQUE
    ...
    else
    {
        ...
        else if (!solid_part)
        {
            ...
            // effect:
            /*
            // FIXME: would like to avoid doing this each time a window is drawn!
            gboolean is_popup_menu = False;
            Atom prop_menu, prop_type, prop, da;
            char *an;
            Display *dpy;
            int di;
            int status;
            ...
            prop_type = XInternAtom(display_info->dpy, "_NET_WM_WINDOW_TYPE", False);
            prop_menu    = XInternAtom(display_info->dpy, "_NET_WM_WINDOW_TYPE_POPUP_MENU", False);
            
            status = XGetWindowProperty(display_info->dpy, cw->id, prop_type, 0L, sizeof (Atom), False,
            ...
            if (status == Success && prop_ret)
            {
                ...
                // debug    
                //an = XGetAtomName(display_info->dpy, prop);
            ...
            */
            ...
            // effect: 
            // if popup menu, whisker, dock, tooltip, or GTK3 CSD
            // window, then use effect
            //
            // maybe: include switch to exclude GTK3 windows?
            // e.g., do below like: (win_has_client && !(cw->c->type & WINDOW_NORMAL) && ! splash)
            //20200813-1 nope - check for popup menu is now commented out as deadcode
            //if (is_popup_menu || (WIN_HAS_CLIENT(cw) && 
            //    !(cw->c->type & WINDOW_SPLASHSCREEN)))
            //{
                // check for non-decorated windows (popups, popovers
                // menus that report a border to gtk_frame_extents
                // and remove border from effect to accommodate them;
                // additionally, check for gtk3 client side decorations
                // (ptui!) and if found, remember widths
                check_gtk_frame_extents(display_info->dpy, cw);

                //(background for menus, whisker menu...)
                // since the panel and whisker menu adjust their own opacity, 
                // just PictOpSrc the effect
                Picture effectMask = solid_picture (cw->screen_info, FALSE,
                                (double) .8,  // .87
                                0.0, 0.0, 0.0);
                
                XRenderComposite (display_info->dpy, PictOpSrc, screen_info->effect_picture, effectMask, paint_buffer,
                                x + cw->gtk_frame_l, y + cw->gtk_frame_t , w - cw->gtk_frame_l - cw->gtk_frame_r, h - cw->gtk_frame_t - cw->gtk_frame_b, 
                                x + cw->gtk_frame_l, y + cw->gtk_frame_t , w - cw->gtk_frame_l - cw->gtk_frame_r, h - cw->gtk_frame_t - cw->gtk_frame_b);
                //20200813-1 deadcode
                //XRenderComposite (display_info->dpy, dark_theme ? PictOpHardLight : PictOpScreen, screen_info->effect_picture, 0, paint_buffer,
                //                x + cw->gtk_frame_l, y + cw->gtk_frame_t , w - cw->gtk_frame_l - cw->gtk_frame_r, h - cw->gtk_frame_t - cw->gtk_frame_b, 
                //                x + cw->gtk_frame_l, y + cw->gtk_frame_t , w - cw->gtk_frame_l - cw->gtk_frame_r, h - cw->gtk_frame_t - cw->gtk_frame_b);
                // adjust opacity for drawing actual window (make it a little 'vibrant')
                //cw->alphaPict = solid_picture (screen_info, FALSE,
                //                (double) cw->opacity * .87 / NET_WM_OPAQUE,  // .87
                //                0.0, 0.0, 0.0);
                //free the effectMask
                if (effectMask)
				{
				    XRenderFreePicture (display_info->dpy, effectMask);
				    effectMask = None;
				}
            //}
			// (foreground for menus, whisker menu, etc.)
            XRenderComposite (display_info->dpy, PictOpOver, cw->picture, cw->alphaPict, paint_buffer,
                               0, 0, 0, 0, x, y, w, h);
            XRenderComposite (display_info->dpy, PictOpLighten, cw->picture, 0, paint_buffer,
                               0, 0, 0, 0, x, y, w, h);
            //the old code for !solid_part
            /*XRenderComposite (display_info->dpy, PictOpOver, 
...
// effect:
/*
//20200815-2 this will causes freezes/crashes if the window tree
// is mutating while XQueryTree is accessing ... deadcode
Window
get_focused_window(Display* dpy)
{
    // get focused window //
    Window focused = None, root = None, *children = 0; 
    unsigned int tmp_u;
    ...
    {
        status = XGetWindowProperty(dpy, focused, 
...
//forward decl
static gboolean
update_paint_effect(gpointer data);

// effect:
// load effect settings from .config/xfce4/xfconf/xfce-perchannel-xml/xfwm-effects.xml
// using xfconfig; this method is also triggered via signal if a property changes
// under that channel; then update_paint_effect if a change occurred 
static void
get_xfconf_settings(ScreenInfo *screen_info)
{

    gboolean setting_changed = False;
    need_load_xfconf_settings = False;
	
	XfconfChannel *window_effects = None;
	window_effects = xfconf_channel_new("xfwm-effects");
	
	if (window_effects == None)
	{
	    printf("error: xfconf_channel_new failed");
	    return;
	}

    // the on-off switch for xfwm effects
    if(xfconf_channel_has_property(window_effects, "/effect-active"))
    {
        gboolean active_changed = xfconf_channel_get_bool(window_effects, "/effect-active", TRUE);
        if (screen_info->use_effect != active_changed)
        {
            screen_info->use_effect = active_changed;
            setting_changed = True;
        }
    }
     
    // the on-off switch for xlib-only (no dependency on imlib2)
    if(xfconf_channel_has_property(window_effects, "/xlib-only"))
    {
        gboolean use_xlib = xfconf_channel_get_bool(window_effects, "/xlib-only", FALSE);
        if (screen_info->xlib_only != use_xlib)
        {
            screen_info->xlib_only = use_xlib;
            setting_changed = True;
            //triggers redraw when setting is changed
            screen_info->effect_update_needed = TRUE;
        }
     }
    // the amount of blur
    if(xfconf_channel_has_property(window_effects, "/blur-amount"))
    {
        signed int blur_value = xfconf_channel_get_int(window_effects, "/blur-amount", 100);
        if (screen_info->blur_amount != blur_value)
        {
            //check bounds
            if (blur_value > 100)
                blur_value = 100;
            if (blur_value < 1)
                blur_value = 1;
            screen_info->blur_amount = blur_value;
            setting_changed = True;
            //triggers redraw of effect when blur slider is changed
            screen_info->effect_update_needed = TRUE;
        }
    }
    // the amount of white added (for light themes)
    if(xfconf_channel_has_property(window_effects, "/white-amount"))
    {
        signed int white_value = xfconf_channel_get_int(window_effects, "/white-amount", 0); 
        if (screen_info->white_amount != white_value)
        {
            //check bounds
            if (white_value > 100)
                white_value = 100;
            if (white_value < 0)
                white_value = 0;
            screen_info->white_amount = white_value;
            setting_changed = True;
        }
    }
    // an image path for 'xray' background effects
    gchar *image_path = "";
    if(xfconf_channel_has_property(window_effects, "/image-path"))
    {
        image_path = xfconf_channel_get_string(window_effects, "/image-path", "\0");
    }
    if (image_path != "\0")
    {
        //printf(">%s<", image_path);
        screen_info->image_path = image_path;
    }

    if (setting_changed)
    {
        update_paint_effect(screen_info);
    }
}

// effect:
// a xfconf_setting changed, so set need_load_xfconf_settings to True
static void
set_need_load_xfconf_settings(gpointer data)
{
    need_load_xfconf_settings = True;
}


// effect:
// method determines if a new or updated effect picture needed

static gboolean
need_effect_update(ScreenInfo *screen_info)
{
    DisplayInfo* display_info = screen_info->display_info;
    Display *dpy = display_info->dpy;
    gboolean effect_update_needed = FALSE;

    //did focus change?
    Window focused = None;
    
    //20200815-2 get_focused_window was causing consistent freezes/crashes
    // because, I think, when multiple windows were becoming visible
    // rapidly, the XQueryTree function was mutating as it was being accessed
    //focused = get_focused_window(dpy);
    
    if (focused != None && focused != screen_info->old_focused)
    {
        //printf("FOCUS %lx \n", focused);
        screen_info->old_focused = focused;
        // NOTE BENE: updating the effect on window focus change is not 
        // necessary; however, if other effects are eventually added, such as, 
        // for example, drawing occluded corners of windows under other 
        // transparent windows and having blur increase with space (deeper 
        // windows more blurred, and desktop most blurred), this would then be 
        // set to TRUE as window focus change would often require redrawing
        effect_update_needed = FALSE;
    }

    //did workspace change?
    if (screen_info->current_ws != screen_info->old_ws)
    {
        //printf("WORKSPACE %1x \n", screen_info->current_ws);
        screen_info->old_ws = screen_info->current_ws;
        //effect_update_needed = TRUE;
    }
    
    //did backdrop (wallpaper) change
    if (screen_info->effect_update_needed)
    {
        screen_info->effect_update_needed = FALSE;
        effect_update_needed = TRUE;
    }

    if (effect_update_needed)
    {
        return TRUE;
    }
    return FALSE;
...
// effect:
//this creates a picture from the xfdesktop window and 
// processes it (blurs it) for use as an 'effects' 
// background that the window compositor uses to render 
// over windows for transluscent window effects
//
//this involves downsizing and upsizing the image while 
// applying a blur filter; there are two methods to do 
// this in the code, depending on the user setting: one 
// uses xlib only while the other uses the imlib2 
// library (should probably have been refactored into 
// two different methods for clarity, but there common 
// code)

void
paint_effect(ScreenInfo *screen_info)
{
    Screen *screen;
    DisplayInfo *display_info;
    Display *dpy;
    GList *list;
    gboolean use_image_path = False, use_xlib_only = False;
    CWindow *cw = NULL;
    XRenderPictFormat *pictfmt;
    int width, height, depth;
    Pixmap original_pixmap = None, tmpPixmap, effect_pixmap, process_pixmap, process_pixmap2;
    Picture original_picture = None, effectPicture, tmpPicture, process_picture, process_picture2;
    Window root;
    Imlib_Image img, smaller_img;
    ...
    // no, really, don't touch //
    pictfmt = XRenderFindVisualFormat(dpy, DefaultVisual (dpy, screen_info->screen)); 
    depth = pictfmt->depth;
    width = screen->width;
    height = screen->height;
    root = RootWindowOfScreen( screen );
    gint screen_width = screen_info->width;
    gint screen_height = screen_info->height;
    //default is 1 for where imlib2 does scaling instead
    double scale = 1; 

    // get xfconf settings, or use some defaults //
    if (need_load_xfconf_settings == True)
    {
        get_xfconf_settings(screen_info);
    }
    //there is an image path, so use that image 
    // instead of making a picture from desktop
    if (strcmp(screen_info->image_path, "\0") != 0)
        use_image_path = True;
    
    // effect is turned off, so remove effect image //
    if (!screen_info->use_effect)
    {
        if (screen_info->effect_picture)
        {
            XRenderFreePicture (dpy, screen_info->effect_picture);
            screen_info->effect_picture = None;
        }
        return;
    }
    
    // no update to effect is needed, so do nothing //
    if (!need_effect_update(screen_info))
        return;
        
	//debug
	//log_time("start paint_effect");
        
    // destroy the old effect_picture so it's not used //
    if (screen_info->effect_picture)
    {
        XRenderFreePicture (dpy, screen_info->effect_picture);
        screen_info->effect_picture = None;
    ...
    //to load a non-blurred background image;
    //only works with imlib2, fails silently :-( for xlib-only
    if (use_image_path && screen_info->xlib_only == False)
    {
        //fprintf(stderr, "file:%s \n", screen_info->image_path);
        //load image from saved path
        img = imlib_load_image( screen_info->image_path );
        if (!img)
			//no point in continuing...bail
            return;
    ...
    {		
	    //last in stack of windows is (probably) 'virtual root' i.e. xfdesktop:
        // CWindow *c = (g_list_last(screen_info->cwindows))->data;
        // however, to make sure we iterate to find Xfdesktop class window parent
        ...
        for (list = g_list_last(screen_info->cwindows); list; list = g_list_previous (list))
        {
            ...
            gboolean is_xfdesktop = False;
            
            //examine window's tree; if desktop, then one subwindow should be of 
            // class Xfdesktop
            if (XQueryTree(dpy, cw->id, &root, &parent, &children, &n))
            {
                if (children)
                {
                    for (int i = 0; i < n; i++)
                    {
                        ...
                        if (XGetClassHint(display_info->dpy, children[i], &class_hints_return))
                        {
                            if (class_hints_return.res_class)
                            {
                                //fprintf(stderr, "\n W:%lx CLASS:%s \n", children[i], class_hints_return.res_class);
                                if (strcmp(class_hints_return.res_class, "Xfdesktop") == 0) // class is correct
                                    is_xfdesktop = True;
                                XFree(class_hints_return.res_class);
                                if (class_hints_return.res_name)
                                    XFree(class_hints_return.res_name); //needs freed as well
                            }
                        }
                    }
                    XFree(children);
            ...
            //we have desktop window, so create a pixmap based on it, then wrap 
            // pixmap in a picture for processing for the effect picture
    
            if (is_xfdesktop)
            {
                XRenderPictFormat *pictfmt = XRenderFindVisualFormat(dpy, DefaultVisual (dpy, screen_info->screen)); 
                int depth = pictfmt->depth;
                //create a pixmap
                original_pixmap = XCreatePixmap(dpy, cw->id, screen_width, screen_height, depth);
                //create a picture
                if (original_pixmap)
                {
                    original_picture = XRenderCreatePicture(dpy, original_pixmap, pictfmt, 0, 0);
                }
 
                // render desktop window to picture //
                
                if (original_picture)
                {
                    Picture desktop_window_picture = get_window_picture (cw);
					
					if (screen_info->xlib_only)
					{		
						// make smaller image to do fast blur //
						double blur_factor = (double) screen_info->blur_amount;
						//if slider is less than one, starts effect process on every 
						// call of paint_all (why?)
						if (blur_factor < 1.0 ) {
							return;
						}
						//this algorithm is meant to make the perception of the 
						// blur settings under xlib-only to match (somewhat) the 
						// behavior of the imlib2 code path (resembles an
						// exponential curve)
						double blur_factor1 = (97 - pow(1.9, ((50 - blur_factor / 2) / 10))) / 100;
						//1.0 would mean picture of zero size
						if (blur_factor1 == 1.0)
							blur_factor1 = .9;
						double blur_factor2 = 1.0 - blur_factor1;
						//scale is what we use in downsizing
						scale = blur_factor2; //for example, .0625 = 1/16 downsample

						//box blur filter
						int kwid = XFixedToDouble(convolution_blur1[0]),
							khei = XFixedToDouble(convolution_blur1[1]);
						XRenderSetPictureFilter(dpy, original_picture, "convolution",
							convolution_blur1, kwid * khei + 2);
		                //scaling matrix
						XTransform xform = {{
		                ...
		                //set scaling to downsize
						XRenderSetPictureTransform( dpy, desktop_window_picture, &xform );
					}
					
					if (desktop_window_picture)
					{
						//render the (possibly downscaled) desktop
						XRenderComposite (myScreenGetXDisplay (screen_info),
								PictOpSrc,
								desktop_window_picture,
								None, original_picture,
								0, 0, screen_width, screen_height, 
								0, 0, screen_width * scale, screen_height * scale);
						XRenderFreePicture (dpy, desktop_window_picture);
						desktop_window_picture = None;
					}
					else
					{
						return;
                ...
                // desktop window found; no need to iterate windows further //
                break;
            }
            else
            {
                //while imlib2 is still processing picture, this can print MANY times
                //fprintf(stderr, "no xfdesktop window found...cannot do effect \n");
            }
        }

        if (!original_pixmap)
        {
            printf("no original_pixmap...cannot do effect \n");
            return;
        }
        
        // downscale and blur the desktop image using imlib2 //
        
		if (screen_info->xlib_only == False)
		{         
			// hand background image to imlib //
		      
			img = imlib_create_image( width, height );
		    
			// set context for drawable image source ('drawable' = window || pixmap) //
		    
			imlib_context_set_image( img );
			imlib_context_set_drawable(original_pixmap);
		    
			// copy pixmap to imlib image //
			...
			// fast blur image with downsize, then boxblur //
			//TODO: we could blur with downsize, then upsize, then paint occluded regions, 
			// then box blur those for multilevel depth
			//in xfconf xml file, blur-effect value is 1 - 100; here scale down is 
			// scale / 10, that is, 10 would be 10/10 = 1
			int scale_down_factor = ceil(screen_info->blur_amount / 10);
			if (scale_down_factor < 1)
			    scale_down_factor = 1;
			    
			int blur_factor = 6;
			if (screen_info->blur_amount < 6)
			{
			    blur_factor = blur_factor = screen_info->blur_amount;
			}
				
			smaller_img = imlib_create_cropped_scaled_image(0, 0, width, height,
				ceil ( width / scale_down_factor ), ceil ( height / scale_down_factor  )); 
		    
			// set context to apply blur to //
			
			imlib_context_set_image( smaller_img );
			imlib_image_blur( blur_factor );
		}
    }

	// make a picture container for tmpPixmap //

    effectPicture = None;
    
    if (screen_info->xlib_only == False)
    {
		// create a pixmap to receive processed image from imlib //
		    
		tmpPixmap = XCreatePixmap(dpy, root, width, height, depth);
	
		// copy image to screensize drawable (in effect, upsizing it) //
	
		imlib_context_set_drawable( tmpPixmap );
		imlib_render_image_on_drawable_at_size( 0, 0, width, height );		
		effectPicture = XRenderCreatePicture(dpy, tmpPixmap, pictfmt, 0, 0);
	}
	else
	{
		//duplicate imlib2's downsizing/upsizing + blur code using xlib //
		
		//below, first we spell out the scale transforms, then we create 
		// the pixmaps/pictures at various sizes and set the convolution 
		// filters, then we render the background by downsizing, then 
		// upsizing in steps while applying box blur; 
		//example resizing steps: .09 > .23 > .50 > 1.0 
		//FIXME: imlib2 code takes 1/100 of a second or so on my system, 
		// while my code takes 1/3 second and twice the cpu...needs to 
		// be faster
		//TODO: look at underlying code for imlib2 to see how they do it

		effect_pixmap = XCreatePixmap(dpy, cw->id, screen_width, screen_height, depth);
		effectPicture = XRenderCreatePicture(dpy, effect_pixmap, pictfmt, 0, 0);
		int kwid = XFixedToDouble(convolution_blur1[0]),
			khei = XFixedToDouble(convolution_blur1[1]);

		double scale1_factor = 2.5;
		if (scale * scale1_factor > 1) {
			scale1_factor = 1;
		} 
		double scale1 = scale * scale1_factor;
		//printf("scale1:%lf \n", scale1);
		//scaling matrix
		XTransform xform1 = {{
			{ XDoubleToFixed( 1 ), XDoubleToFixed( 0 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 1 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 0 ), XDoubleToFixed( scale1_factor ) }
		}};
		...
		XTransform xform2 = {{
			{ XDoubleToFixed( 1 ), XDoubleToFixed( 0 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 1 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 0 ), XDoubleToFixed( .5 / scale1 ) }
		}};
		...
		XTransform xform3 = {{
			{ XDoubleToFixed( 1 ), XDoubleToFixed( 0 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 1 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 0 ), XDoubleToFixed( 1 / .5 ) }
		}};

		process_picture = None;
		process_pixmap = XCreatePixmap(dpy, cw->id, screen_width * scale1, screen_height * scale1, depth);  //4
		if (process_pixmap) {
			process_picture = XRenderCreatePicture(dpy, process_pixmap, pictfmt, 0, 0);
		}
		
		XRenderSetPictureFilter(dpy, process_picture, "convolution",
				convolution_blur1, kwid * khei + 2);

		process_picture2 = None;
		process_pixmap2 = XCreatePixmap(dpy, cw->id, screen_width * scale2, screen_height * scale2, depth);
		if (process_pixmap2) {
			process_picture2 = XRenderCreatePicture(dpy, process_pixmap2, pictfmt, 0, 0);
		}
		
		XRenderSetPictureFilter(dpy, process_picture2, "convolution",
				convolution_blur1, kwid * khei + 2);

		XRenderSetPictureTransform( dpy, original_picture, &xform1 );  
		
		XRenderComposite(dpy, PictOpSrc, original_picture, None, process_picture,
			0, 0, screen_info->width * scale, screen_info->height * scale,
			0, 0, screen_width * scale1, screen_height * scale1);

		XRenderSetPictureTransform( dpy, process_picture, &xform2 ); 

		XRenderComposite(dpy, PictOpSrc, process_picture, None, process_picture2,
			0, 0, screen_info->width * scale1, screen_info->height * scale1,
			0, 0, screen_width * scale2, screen_height * scale2);

		XRenderSetPictureTransform( dpy, process_picture2, &xform3 );

		XRenderComposite(dpy, PictOpSrc, process_picture2, None, effectPicture,
			0, 0, screen_width * scale2, screen_height * scale2,
			0, 0, screen_info->width, screen_info->height);

		//scaling matrix
		XTransform xform_normalize = {{
			{ XDoubleToFixed( 1 ), XDoubleToFixed( 0 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 1 ), XDoubleToFixed( 0 ) },
			{ XDoubleToFixed( 0 ), XDoubleToFixed( 0 ), XDoubleToFixed( 1 ) }
		}};

		//reset filter and normalize (null out) transform on effectPicture
		XRenderSetPictureFilter(dpy, effectPicture, "Nearest", NULL, 0);
		XRenderSetPictureTransform( dpy, effectPicture, &xform_normalize );
    }
    
    /* for screen debugging */
    /*effectPicture = solid_picture (screen_info, TRUE,
                        1.0,  // alpha
                        0.0,  // red
                        1.0,  // green
                        0.0); // blue */
                        
    //screen_info->white_amount > 0 means user indicates light theme is being
    // used, not dark theme, so we add some white to make effect less jarring                    
    if ((screen_info->white_amount > .001) && !use_image_path)
    {
        gdouble color_value = (gdouble) screen_info->white_amount / 100;
        tmpPicture = None;
        //create 'white' picture
        tmpPicture = solid_picture (screen_info, TRUE,
                            0.1,  // alpha //0.5
                            color_value,  // red
                            color_value,  // green
                            color_value); // blue ; rgb == orange
		//alpha mask
		cw->alphaPict = solid_picture (screen_info, FALSE,
				(double) cw->opacity * .5 / NET_WM_OPAQUE,
				0.0, 0.0, 0.0);
        //paint picture onto the picture-buffer
        XRenderComposite (dpy, PictOpOver, tmpPicture, cw->alphaPict, 
                            effectPicture, 
                            0, 0, 0, 0, 0, 0,
                            screen_info->width,
                            screen_info->height);
    ...
    // cache new effect picture //
    
    if (effectPicture)
    {
        screen_info->effect_picture = effectPicture;
        screen_info->effect_updated = TRUE;
    }
    else
    {
        printf("could not set effect_picture...cannot do effect \n");
    }

    // cleanup //

	//clean up specific to imlib2
	if (screen_info->xlib_only == False)
	{
		imlib_free_image(); //free the smaller_img
		imlib_context_set_image (img);
		imlib_free_image(); //free the img
		XFreePixmap(dpy, tmpPixmap);
	}
	//clean up specific to xlib-only
	else
	{
	
		if (effect_pixmap)
		{
			XFreePixmap(dpy, effect_pixmap);
			effect_pixmap = None;
		}
		//clear out tmpPixmap / tmpPicture
		if (tmpPicture)
		{
			XRenderFreePicture (dpy, tmpPicture);
			tmpPicture = None;
		}
		//clear out process pixmap / picture
		if (process_pixmap)
		{
			XFreePixmap(dpy, process_pixmap);
			process_pixmap = None;
		}
		if (process_picture)
		{
			XRenderFreePicture (dpy, process_picture);
			process_picture = None;
		}
		//clear out process pixmap2 / picture2
		if (process_pixmap2)
		{
			XFreePixmap(dpy, process_pixmap2);
			process_pixmap2 = None;
		}
		if (process_picture2)
		{
			XRenderFreePicture (dpy, process_picture2);
			process_picture2 = None;
		}

	}
	//clean up that needs to happen in either case
    if (original_pixmap)
    {
        XFreePixmap(dpy, original_pixmap);
        original_pixmap = None;
    }
    if (original_picture)
    {
        XRenderFreePicture (dpy, original_picture);
        original_picture = None;
    }
    
	//debug
	//log_time("end paint_effect");
...
{
    DisplayInfo *display_info;
    XserverRegion paint_region, effect_region; //effect
    ...
    myDisplayErrorTrapPush (display_info);
    //log_time("start paint_all");
    // effect:
	paint_effect(screen_info);

    // ---------- make, cache a pixmap of screensize ---------- //
    // the pixmap is a (here: offscreen) drawing surface
    ...
    // ---------- make, cache a picture of above pixmap ---------- //
    // the picture if a compositing container for the pixmap
   ...
   // ---------- copy damaged REGION rect ---------- //
   // paint_all is passed 'region', which is 'damaged' rect in screen that is
   // union of all damage that needs repainting
   // so: create region, copy paint_region, and eventually destroy 
   // when no longer needed
    ...
    // ---------- iterate CWindows topmost to bottom ---------- //
    // mostly does: get_window_picture, then paint_win
    // note: CWindows are containers for windows (window IDs) that
    // have all the extra needed info, like opacity, etc.
    ...
    // last window to iterate here is usually desktop...
    for (list = screen_info->cwindows; list; list = g_list_next (list))
    {
        cw = (CWindow *) list->data;
        
        // ----------  no redraw needed... ---------- //
        // note: "continue" (in c language) forces next iteration of the for loop
        ...
        // ----------  paint the window with PAINT_WIN ---------- //
        // nothing happens here when compositor is active
     ...
     */
    // ie, paint_win subtracts fixed rects from region at end of method

    // --------- if stuff to repaint, paint root ---------- //
    // here we reapply damaged region initially passed to paint_win
    // note: XRegion contains 1) list of rects and 2) total bounds
    
    XFixesSetPictureClipRegion (dpy, paint_buffer, 0, 0, paint_region);
    if (!is_region_empty (dpy, paint_region))
    {
        // paints cached picture of 'root' window (made from image data
        // stored as property of that window especially) onto the drawing
        // buffer
        //
        // paint_win substracts painted window rects from paint_region, so
        // this repaints everything else (before 'transparent' windows
        // painted in next 'for' statement
        //
        // note: apparently, regions can be 'hollowed out' rectangles, for
        // painting e.g. shadows around windows
    ...
    for (list = g_list_last(screen_info->cwindows); list; list = g_list_previous (list))
    {
        ...
        // ---------- no paint needed (cos skipped last time too) ---------- //
        if (cw->skipped)
        {
        ...
        // ---------- paint shadow around window frame ---------- //
        ...
        if (cw->shadow)
        {
            ...
            // the black picture is composited using the alpha mask 'shadow'
...
#endif /* HAVE_EPOXY */
    {
        ...
        {
            // --- final step: composite paint buffer onto root buffer --- //
...
add_win (DisplayInfo *display_info, Window id, Client *c)
{
	...
	new->gtk_frame_t = 0;
	new->gtk_frame_b = 0;
	new->has_gtk_frame_extents = False;
...
// effect:
//
// because xfdesktop (from which we get backdrop) updates
// backdrop lazily after workspace switch, we damage screen
// and set effect as needing update, so everything redraws
// in a mostly timely way. This fixes the problem where parts
// of the screen are outdated and don't redraw
...
//200 miliseconds
static gboolean
update_paint_effect_quicker(gpointer data)
{
    ScreenInfo *screen_info = (ScreenInfo*) data;
    if (!screen_info)
        return FALSE;
...
compositorHandlePropertyNotify (DisplayInfo *display_info, XPropertyEvent *ev)
{
    ...
    for (p = 0; p < 2; p++)
    {
        if (ev->atom == backgroundProps[p] && ev->state == PropertyNewValue)
        {
            ...
            // effect:
            // the backdrop (the background image) changed because 
            // the workspace changed or the user changed the image
            
            screen_info->effect_update_needed = TRUE;
            
            // .2 second delay to catch quick update of background (looks
            // better to user; strictly speaking, only the 500 ms timer is 
            // necessary; False is returned, destroying timer
            g_timeout_add(200, update_paint_effect_quicker, screen_info);
            // .5 second delay to catch slow update of background (those
            // missed by "quicker" method
            g_timeout_add(500, update_paint_effect, screen_info);
...
compositorHandleExpose (DisplayInfo *display_info, XExposeEvent *ev)
{
	...
	//fprintf(stderr, "win:%lu w:%d h:%d \n", cw->id, rect[0].width, rect[0].height);
...
compositorManageScreen (ScreenInfo *screen_info)
{
    ...
    // effect:
    // signal when setting for effect is changed externally 
    XfconfChannel *window_effects = xfconf_channel_new("xfwm-effects");
    g_signal_connect(G_OBJECT(window_effects), "property-changed",
                         G_CALLBACK(set_need_load_xfconf_settings), NULL);
